...
/** Writer for {@link PLYFormat#BINARY_BIG_ENDIAN} encoded PLY data.
 *  
 *  @author Dirk Toewe
 */
class DataStreamOutBE extends DataStreamOut
{
// FIELDS
...
// CONSTRUCTORS
  DataStreamOutBE( OutputStream _out ) { out =_out; }
...
// METHODS
  ...
  {
    cache[0] = (byte) (uint16 >>> 8);
    cache[1] = (byte) (uint16 >>> 0);
    out.write(cache,0,2);
  }
  ...
  {
    cache[0] = (byte) (uint32 >>>24);
    cache[1] = (byte) (uint32 >>>16);
    cache[2] = (byte) (uint32 >>> 8);
    cache[3] = (byte) (uint32 >>> 0);
    out.write(cache,0,4);
  }
  ...
  @Override void writeFloat64( double float64 ) throws IOException
  {
    ...
    cache[0] = (byte) (bits >>>56);
    cache[1] = (byte) (bits >>>48);
    cache[2] = (byte) (bits >>>40);
    cache[3] = (byte) (bits >>>32);
    cache[4] = (byte) (bits >>>24);
    cache[5] = (byte) (bits >>>16);
    cache[6] = (byte) (bits >>> 8);
    cache[7] = (byte) (bits >>> 0);
    out.write(cache,0,8);
  }
